...
export default defineConfig(({ mode }) => {
 ...
 return {
   plugins: [react(), localQuestionApi(env), localPhotoRecognitionApi(env)],
 };
});
...
function localPhotoRecognitionApi(env: Record<string, string>): Plugin {
  return {
    name: "local-photo-recognition-api",
    configureServer(server) {
      server.middlewares.use("/api/recognize-photo", async (request, response) => {
        response.setHeader("Content-Type", "application/json; charset=utf-8");
        response.setHeader("Cache-Control", "no-store");
        ...
        try {
          ...
          if (!imageDataUrl.startsWith("data:image/")) {
            response.statusCode = 400;
            response.end(JSON.stringify({ error: "请求缺少图片数据。" }));
            return;
          }
          if (Buffer.byteLength(imageDataUrl, "utf8") > 3_800_000) {
            response.statusCode = 413;
            response.end(JSON.stringify({ error: "图片请求体过大。请裁剪照片或降低图片尺寸后重试。" }));
            return;
          }

          const mode = body.mode === "holdem" || body.mode === "omaha" || body.mode === "auto" ? body.mode : "auto";
          const recognition = await recognizePokerPhotoWithOpenAI({
            apiKey,
            model: env.OPENAI_VISION_MODEL || env.OPENAI_MODEL || env.VITE_OPENAI_MODEL || process.env.OPENAI_MODEL || process.env.VITE_OPENAI_MODEL,
            imageDataUrl,
            mode: mode as PokerGameMode,
          });
          response.end(JSON.stringify({ recognition }));
        } catch (error) {
          response.statusCode = getPokerVisionErrorStatus(error);
          response.end(JSON.stringify(toPokerVisionErrorPayload(error)));
        }
      });
    },
  };
